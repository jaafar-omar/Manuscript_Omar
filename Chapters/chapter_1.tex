...
This study presents the development of a volumetric measurement system for flour storage bin. The system is designed to be controlled and scanned remotely using a web-based application.
...
The volume of stored grain is often calculated using depth measurements, a method used across various industries. Once the grain's height is determined through depth measurement and surface profile assessment, this height is multiplied by the base area of the storage container to calculate the volume:
...
For more accurate volume estimations, advanced methods like Delaunay triangulation, voxelization, and Convex Hull algorithms are used. These computational geometry techniques provide precise measurements by effectively representing complex shapes and reducing processing time. By integrating these advanced methods, the reliability and accuracy of volume calculations can be significantly enhanced.
...
Where $x$, $y$, and $z$ represent the Cartesian coordinates of the point.
$\rho$ is the distance radian between the point and the origin. The polar angle
$\theta$ is calculated from the point to the positive $z$-axis. Azimuthal angle $\phi$ is defined as the angle between the positive $x$-axis to the point's projection onto the $xy$-plane.
...
\begin{enumerate}
	...
	\item \textbf{Point Cloud} \textemdash refers to a collection of data points located in a three-dimensional space, usually depicting the surface of an object. Each point is specified by its coordinates in three dimensions (x, y, and z) and may also contain supplementary information such as color, intensity, or normal vector.
